...
- `MAX_QUEUED_WORKFLOW_RUNS=10` — global cap on GitHub Actions runs sitting in `queued` (waiting to start). Both `/api/bulk` and `/admin/retry-unknowns` query the GitHub Actions REST API for the workflow's current queued count and return 429 if it's already at the cap. The workflow's own `concurrency:` group still caps in-flight runs at 3 — this setting caps the *wait line*. `0` disables the check.
